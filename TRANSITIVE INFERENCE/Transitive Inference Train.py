...
class Trial(object):
    ...
    def newBlock(self):
        ...
        if self.AB_correct_pct >= 0.80 and self.BC_correct_pct >= 0.80 and self.CD_correct_pct >= 0.80 and self.DE_correct_pct >= 0.80 and self.XX_correct_pct >= 0.80:
            self.consecutive += 1
            #print("Congrats! One block passed with above 80%")
            #print("self.consecutive = " + str(self.consecutive))
        else:
            self.consecutive = 0

        if self.step_wise_phase <= 8 and self.consecutive == 2:
            self.step_wise_phase += 1
            self.consecutive = 0
            #print("Phase Passed")
            #print("Moving to Phase: " + str(self.step_wise_phase))

        elif self.step_wise_phase == 9 and self.consecutive == 2:
            pygame.quit()
...
pygame.display.set_caption("Transitive Inference Training")
